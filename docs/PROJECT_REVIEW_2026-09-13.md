...
Start with a modest crop encoder and temporal fusion head, retaining the existing landmark model. Compare masked-hand RGB, silhouette and landmark-only conditions. Use the user's synchronized keyboard recordings for supervision and desk phrases for adaptation. Test a frozen pretrained visual encoder against a small encoder trained on these crops; the repository's failed public-footage transfer is a reason to measure domain transfer, not assume it works. Avoid training a large video model from scratch on 31 phrases.
